...
Page({
  ...
  bindLoadMore: function () {
    ...
    var nextMonth = (parseInt(varMonth) + 1);
    if (prevMonth <= 0) {
      prevMonth = 12;
      prevYear--;
    }
  ...
  onLoad: function (options) {
    ...
    //验证页面数据
    if (isNaN(varYear) || isNaN(varMonth)) {
      console.log("加载页面数据错误:", varYear, varMonth);
      wx.showModal({
        title: '加载页面错误',
...
function JsonToList(ListData) {
  ...
  getData(jsonData, function (ret) {
    if (ret.uid > 0) {
      ...
      while (titleIndex-- > 0) {
        if (json[titleIndex].isTitle == true) {
          json[titleIndex].overMoney = getApp().ValueToMoney(ret.data.SumInMoney - ret.data.SumOutMoney);
          break;
        }
      }
